...
const Label = ({ children }: { children: React.ReactNode }) => (
  <div style={{
    ...
    color: 'var(--db-text-dim)', marginBottom: 14,
  }}>
    {children}
  </div>
)
...
const Panel = ({ children, style }: { children: React.ReactNode; style?: React.CSSProperties }) => (
  <div style={{
    ...
    borderRadius: 'var(--db-radius-lg)', padding: '18px 20px', ...style,
  }}>
    {children}
  </div>
)
...
// ── OVERVIEW ──────────────────────────────────────────────────────────────────
function Overview() {
  const { assets, stats, loading } = useOverviewData()

  const totalAssets  = assets.length
  const publicAssets = assets.filter(a => (a.visibility || 'public') === 'public').length
  ...
  return (
    <div style={s.main}>
      ...
      {/* Fall season banner */}
      <div style={s.fallBand}>
        <span style={{ fontSize: 16 }}>🍂</span>
        <div style={{ flex: 1 }}>
          <div style={s.fallTitle}>{SEASON.label} · Booking Open</div>
          <div style={s.fallSub}>{SEASON.cta}</div>
        </div>
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          <a href="https://antcpu.com/manda/agent/" style={s.fallBtn} target="_blank" rel="noreferrer">
            Agent →
          </a>
          <a href="https://antcpu.com/manda/" style={s.fallBtnSecondary} target="_blank" rel="noreferrer">
            Portfolio →
          </a>
        </div>
      </div>
          ...
          {CATEGORIES.map(c => {
            ...
            return (
              <div key={c.id} style={{
                ...
                <span style={{ fontSize: 11, color: 'var(--db-text)', fontWeight: 600 }}>
                  {c.label}
                </span>
                <span style={{
                  fontSize: 10,
...
function Portfolio() {
  ...
  return (
    <div style={s.main}>
      <Panel>
        <Label>
          Portfolio — {loading ? '···' : `${assets.length} asset${assets.length !== 1 ? 's' : ''}`}
        </Label>
        {loading ? (
...
function Uploads() {
  return (
    <div style={s.main}>
      ...
      <Panel>
        <Label>🔍 Search After Upload</Label>
        <SearchPanel />
      </Panel>
    </div>
  )
...
function Vault() {
  ...
  async function remove(id: string, filename: string) {
    if (!confirm(`Remove "${filename}"? (Blob + Cloudinary not deleted)`)) return
    setActing(id)
    try {
      const res = await fetch(`${API.assets}/${id}`, {
    ...
    <div style={s.main}>
      {toast && (
        <div style={{
          ...
          fontSize: 12, fontFamily: 'var(--db-font-mono)',
          color: '#c8f564', zIndex: 999,
        }}>
          {toast}
        </div>
      )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--db-text-dim)', fontFamily: 'var(--db-font-mono)', marginTop: -10 }}>
              Stage privately · assign to partners · release when ready
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
            {[
              ...
              <div key={label} style={{
                fontSize: 10, fontFamily: 'var(--db-font-mono)', color,
        ...
        <Panel>
          ...
          {privateAssets.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {privateAssets.map(a => (
                <VaultRow
                  key={a.id} asset={a} acting={acting === a.id} partners={PARTNERS}
        ...
        <Panel>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {publicAssets.map(a => (
              <div key={a.id} style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '8px 10px', background: 'var(--db-surface2)',
                borderRadius: 8, border: '1px solid var(--db-border)',
              }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
...
const ROADMAP: Record<string, { eta: string; desc: string }> = {
  ...
  settings:  { eta: 'v0.3', desc: 'Platform settings, API keys & preferences' },
  deploy:    { eta: 'v0.4', desc: 'Deployment history and build logs' },
}

function ComingSoon({ name }: { name: string }) {
  const info = ROADMAP[name] ?? { eta: 'v0.3', desc: 'Coming soon' }
  return (
    <div style={s.main}>
      <Panel>
        <Empty
          ...
          action={
            <a href="/wiki/roadmap" style={s.emptyAction}>
              View Roadmap → {info.eta}
            </a>
          }
        />
      </Panel>
      ...
      <div style={s.topnav}>
        ...
        {[
          ...
          <a
            ...
            style={s.navBtn}
            onMouseEnter={e => {
              e.currentTarget.style.borderColor = label.includes('🍂')
                ? 'rgba(240,165,0,0.4)'
                : 'rgba(200,245,100,0.3)'
              e.currentTarget.style.color = label.includes('🍂') ? '#f0a500' : '#c8f564'
            }}
            onMouseLeave={e => {
